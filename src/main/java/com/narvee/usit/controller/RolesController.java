...
public class RolesController {
	...
	@ApiResponses({ @ApiResponse(code = 200, message = "Fetched all roles"),
			@ApiResponse(code = 404, message = "Entities not found"),
			@ApiResponse(code = 500, message = "Internal Server error") })
	@RequestMapping(value = "/all2/{page}/{size}", method = RequestMethod.GET, produces = "application/json")
	public ResponseEntity<RestAPIResponse> getAllRolesByPaging(@PathVariable int page, @PathVariable int size) {
		logger.info("inside RolesController.getAllRoles()=> fetching all roles");
		// List<Roles> saveroles = Service.getAllRoles();
		int tsize = page - 1;
		Page<Roles> productWithPagination = Service.findProductWithPagination(tsize, size);
