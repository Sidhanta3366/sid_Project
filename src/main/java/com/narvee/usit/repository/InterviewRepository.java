...
public interface InterviewRepository extends JpaRepository<Interview, Serializable> {
	...
	@Query(value = "select s.subid, c.consultantname,  s.position,\r\n"
	...
	@Query(value = " select v.company as vendor,i.flg, s.endclient,c.consultantid as consid, c.consultantname as name, i.interview_date, i.round, i.mode,  s.created_date,\r\n"
			...
			+ "  s.consultantid = c.consultantid and s.vendorid=v.id and i.addedby = u.userid  and i.flg=:flg and c.status='Active' order by i.updated_date desc", nativeQuery = true)
	public List<ListInterview> getall(String flg);
	
	@Query(value = " select v.company as vendor,i.flg, s.endclient,c.consultantid as consid, c.consultantname as name, i.interview_date, i.round, i.mode, s.vendor, s.created_date,\r\n"
			+ " u.userid, u.fullname,u.pseudoname, i.interview_status, i.time_zone, i.id as intrid, s.subid\r\n"
			+ " from interview i, consultant_info c, submission s , users u, vendor v where i.submissionid = s.subid and\r\n"
			+ "  s.consultantid = c.consultantid and s.vendorid=v.id and i.addedby = u.userid  and i.flg=:flg and u.userid=:userid and c.status='Active' order by i.updated_date desc", nativeQuery = true)
	public List<ListInterview> getallbyRole(String flg,long userid);

	@Query(value = " select v.company as vendor,i.flg,s.endclient,c.consultantid as consid, c.consultantname as name, i.interview_date, i.round, i.mode, s.vendor, s.created_date,\r\n"
			+ " u.userid, u.fullname,u.pseudoname, i.interview_status, i.time_zone, i.id as intrid, s.subid\r\n"
			+ " from interview i, consultant_info c, submission s , users u , vendor v where i.submissionid = s.subid and\r\n"
			+ "            s.consultantid = c.consultantid and s.vendorid=v.id and i.addedby = u.userid", countQuery = "SELECT count(*) FROM interview ", nativeQuery = true)
	...
	@Query(value="select v.company as vendor, i.id,c.consultantname as name, c.consultantemail as email, s.position as jobtitle,s.projectlocation as location,i.interview_date, i.interview_status,i.mode, \r\n"
			+ "i.round,i.time_zone as timezone from consultant_info c , interview i, submission s, vendor v   where \r\n"
			+ " s.vendorid=v.id and s.consultantid = c.consultantid and i.submissionid = s.subid and i.id = :id", nativeQuery = true)
