...
public interface IConsultantFileUploadsRepository extends JpaRepository<ConsultantFileUploads, Serializable> {
	...
	@Query(value = "select consultantname from consultant_info c , consultant_file_uploads f where f.consultant_id = c.consultantid and docid=:id",nativeQuery=true)
	public String findBydocid(@Param("id")long id);
