...
public interface ISubmissionRepository extends JpaRepository<Submissions, Serializable> {
	...
	@Query(value = "select  u.userid,u.fullname,u.pseudoname,c.consultantname,c.consultantid, s.created_date as createddate ,\r\n"
			...
			+ "where v.id = s.vendorid and s.consultantid = c.consultantid and u.userid = s.userid  and flg=:flag and c.status='Active' order by s.subid desc", nativeQuery = true)
	public List<SubmissionDTO> getsalessubmission(@Param("flag") String flag);
