...
public interface IUsersRepository extends JpaRepository<Users, Long> {
	...
	@Query(value = "select email from users where TIMESTAMPDIFF(DAY,last_login,NOW()) >= 1 or last_login is null", nativeQuery = true)
	public String[] getDailyLoginStatus();
...
//	public String[] findsalesrecruiterMail();
	...
	/// mails for teamlead sent to team, tl, manager   SELECT u.email FROM Users u WHERE  u.teamlead=19 or userid in (19,18)
	
	@Query("SELECT u.email FROM Users u WHERE  u.teamlead=:userid or userid in (:userid,:managerid)")
	public String[] teamleadmails(@Param("userid")  long userid,@Param("managerid")  long managerid);
	...
	@Query(value = "select email from users where department in ('Bench Sales')", nativeQuery = true)
