...
//			return true;
	...
	public int unlockUser(Users users) {
		...
		//lockedUser.setLastLogout(logoutTime);							//commented to remove security feature
		iUsersRepo.save(lockedUser);
		return 0;
	}
	
	//public List<UserInfoDTO> getDailyLoginStatus();
	
	// @Scheduled(fixedRate=10000)
	@Scheduled(cron = "0 0 19 ? * MON-FRI") // runs for every 2 day's
	public void scheduleTaskForDialyLogin() {
		String[] list = iUsersRepo.getDailyLoginStatus();
		if(list.length>0) {
			emailService.gentleReminderTologin(list);
		}
